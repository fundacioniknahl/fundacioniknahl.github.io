/* ============================================================
   FUNDACIÓN IKNAHL — JAVASCRIPT PRINCIPAL
   main.js
   ============================================================ */
...
function renderServicios() {
  ...
  const svgIcons = {
    orientacion: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><circle cx="12" cy="12" r="10"/><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76"/></svg>',
    refugio: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>',
    derechos: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z"/></svg>',
    acompanamiento: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>',
    canalizacion: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 010 14.14M15.54 8.46a5 5 0 010 7.07"/></svg>',
    capacitacion: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M2 3h6a4 4 0 014 4v14a3 3 0 00-3-3H2z"/><path d="M22 3h-6a4 4 0 00-4 4v14a3 3 0 013-3h7z"/></svg>',
    comunidad: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><circle cx="8" cy="9" r="3"/><circle cx="16" cy="9" r="3"/><path d="M3 20c0-3 2.5-5 5-5s5 2 5 5"/><path d="M11 20c0-3 2.5-5 5-5s5 2 5 5"/></svg>',
  };

  container.innerHTML = CONFIG.servicios.map((s, i) => {
    const icon = svgIcons[s.icono] || svgIcons['orientacion'];
    return `
  ...
  const perfilesGenericos = [
    {
      ...
      cargo: 'Equipo humanitario',
      desc: 'Personas dedicadas al acompañamiento directo y a la atención inmediata de quienes buscan protección y orientación.'
    },
    {
      icono: '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" aria-hidden="true"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',
      cargo: 'Orientación y acompañamiento',
      desc: 'Encargados de brindar información clara y acompañamiento a personas migrantes, refugiadas y solicitantes de protección.'
    },
    {
    ...
    },
    {
      ...
      cargo: 'Voluntariado',
      desc: 'Red de personas voluntarias que contribuyen con su tiempo y conocimiento a nuestra labor humanitaria.'
    },
  ];
...
function renderNoticias() {
  ...
  // Cards de noticias — "próximamente" cuando aún no hay fecha real
  const realCards = noticias.map((n, i) => {
    const esProximamente = !n.fecha || n.proximamente;
    const dateOrBadge = esProximamente
      ? `<div class="news-date">Próximamente</div>`
      : `<time class="news-date" datetime="${n.fecha}">${formatDate(n.fecha)}</time>`;
    const tag = esProximamente ? 'div' : 'a';
    const hrefAttr = esProximamente ? '' : `href="${n.enlace}"`;
    return `
    <${tag} class="news-card${esProximamente ? ' placeholder' : ''} fade-in delay-${i + 1}" ${hrefAttr} aria-label="${esProximamente ? n.titulo : 'Leer: ' + n.titulo}">
      <div class="news-image">
        <div class="news-image-inner"${esProximamente ? ' style="background:rgba(18,48,71,0.15);"' : ''}>
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="${esProximamente ? 'rgba(18,48,71,0.5)' : 'white'}" stroke-width="1.5" opacity="${esProximamente ? '1' : '0.7'}" aria-hidden="true"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
        </div>
        <span class="news-category"${esProximamente ? ' style="background:rgba(18,48,71,0.15);color:var(--color-primary);"' : ''}>${esProximamente ? 'Próximamente' : 'Actividades'}</span>
      </div>
      <div class="news-body">
        ${dateOrBadge}
        <h3 class="news-title"${esProximamente ? ' style="color:var(--color-muted);"' : ''}>${n.titulo}</h3>
        <p class="news-excerpt"${esProximamente ? ' style="color:var(--color-muted-light);"' : ''}>${n.resumen}</p>
        ${esProximamente ? '' : '<span class="news-read-more">Leer más →</span>'}
      </div>
    </${tag}>`;
  }).join('');
...
function renderWhatsApp() {
  ...
  if (!CONFIG?.contacto?.whatsapp) {
    // Sin número configurado todavía: no abrir un número falso.
    links.forEach(el => {
      ...
      const span = el.querySelector('span:not(.footer-contact-icon)');
      if (span) span.textContent = 'WhatsApp próximamente disponible';
      else el.textContent = 'WhatsApp próximamente disponible';
    });
    return;
  }
...
function renderValores() {
  ...
  const svgIcons = {
    justicia: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M12 3L2 7l10 4 10-4-10-4z"/><path d="M2 17l10 4 10-4"/><path d="M2 12l10 4 10-4"/></svg>',
    solidaridad: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>',
    confidencialidad: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>',
    proteccion: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',
    dignidad: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>',
    esperanza: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><circle cx="12" cy="17" r="4"/><line x1="12" y1="3" x2="12" y2="9"/><line x1="4.22" y1="10.22" x2="6.34" y2="12.34"/><line x1="19.78" y1="10.22" x2="17.66" y2="12.34"/><line x1="1" y1="17" x2="4" y2="17"/><line x1="20" y1="17" x2="23" y2="17"/></svg>',
  };

  container.innerHTML = CONFIG.org.valores.map((v, i) => {
    const icon = svgIcons[v.icono] || svgIcons['dignidad'];
    return `
